...
print("Login SB SUCCESS ")

poll = LinePoll(cl)
call = cl
creator = ["u1dfa218227cfbff6154ebd9be34b4182"]
owner = ["u1dfa218227cfbff6154ebd9be34b4182"]
admin = ["u1dfa218227cfbff6154ebd9be34b4182"]
...
wait = {
    ...
    "sticker":False,
    "selfbot":True,
    "mention":"ᴏʏ @! ɴɢɪɴᴛɪᴘ ᴍᴜʟᴜ sɪʜ.. \nsɪɴɪ ᴍᴀsᴜᴋ..😂",
    "Respontag":"Dih Kepo Ya Tag q...",
    "welcome":"┝♽ᴡᴇʟᴄᴏᴍᴇ ʙᴀʙʏ..\n┝♽ ᴘʟᴇᴀsᴇ ᴄᴇᴋ ɴᴏᴛe\n┕─────────────── ",
            ...
            if mid in op.param3:
                if wait["autoJoin"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin:
                        cl.acceptGroupInvitation(op.param1)
                        ginfo = cl.getGroup(op.param1)
                        cl.sendMessage(op.param1,"Terimakasih Kk Legend Sudah dijepit ke " +str(ginfo.name))
                    else:
                        cl.acceptGroupInvitation(op.param1)
                        ginfo = cl.getGroup(op.param1)
                        cl.sendMessage(op.param1,"Terimakasih Kk Legend Sudah dijepit ke " + str(ginfo.name))
               ...
               if msg.contentType == 0:
                    ...
                    if text is None:
                        ...
                        if cmd == "self on":
                            if msg._from in admin:
                                wait["selfbot"] = True
                                cl.sendMessage(msg.to, "sᴇʟғ Ready")
                                
                        elif cmd == "self off":
                            if msg._from in admin:
                                wait["selfbot"] = False
                                cl.sendMessage(msg.to, "sᴇʟғ Turn Off")
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                cl.sendImageWithURL(msg.to,"http://dl.profile.line-cdn.net/")

                        elif cmd == "creator" or text.lower() == 'creator':
                            if msg._from in admin:
                                cl.sendMessage(msg.to,"🔳🔳ᴄʀᴇᴀᴛᴏʀ Cabeˢᴱᴸᶠ🔲🔲") 
                                ma = ""
                                for i in creator:
                                    ma = cl.getContact(i)
                        ...
                        elif cmd == "kenizawa" or text.lower() == 'b':
                                 tz = pytz.timezone("Asia/Jakarta")
                                 timeNow = datetime.now(tz=tz)
                                 rey = "ᴋᴇɴɪᴢᴀᴡᴀ 「ケニザワ」\nᵀᴱᴬᴹsᴇʟғʙᴏᴛ"
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               ...
                               cl.sendMessage(msg.to,"╔══[ CABE ғʀɪᴇɴᴅ ʟɪsᴛ ]\n║\n"+ma+"║\n╚══[ Total「"+str(len(gid))+"」Friends ]")
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               ...
                               for i in gid:
                                   ...
                                   ma += "╠ " + str(a) + ". " +G.name+ "\n"
                               cl.sendMessage(msg.to,"╔══[ KILLER ɢʀᴏᴜᴘ ʟɪsᴛ ]\n║\n"+ma+"║\n╚══[ Total「"+str(len(gid))+"」Groups ]")
...
#====================================================================   
                           ...
                           if msg._from in admin:
                                try:
                                    ...
                                    cl.sendMessage(msg.to, 'ᴅᴏɴᴇ ʟɪᴋᴇ ᴀɴᴅ ᴄᴏᴍᴍᴇɴᴛ '+str(len(st))+' ᴘᴏsᴛ ғʀᴏᴍ' + str(s))
                                except Exception as e:
                                    cl.sendMessage(msg.to, str(e))
...
#===========BOT UPDATE============#
                        elif cmd == "malaikat" or text.lower() == 'yes':
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                          ...
                          if wait["selfbot"] == True:
                           if msg._from in admin:
                             if msg.toType == 2:
                                ...
                                jmlh = int(wait["limit"])
                                cl.sendMessage(msg.to, "Berhasil mengundang {} undangan Call Desah".format(str(wait["limit"])))
                                if jmlh <= 1000:
                                  for x in range(jmlh):
                                     try:
